...
for name, arr in [("X_features", X), ("y_target", y), ("X_train", X_train), ("X_test", X_test), ("y_train", y_train), ("y_test", y_test)]:
    np.save(f"{name}.npy", arr)

print(f"Done ✅ — X: {X.shape}, train/test split: {X_train.shape[0]}/{X_test.shape[0]}")