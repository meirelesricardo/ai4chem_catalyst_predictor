...
# RXNFP encodes the full reaction SMILES (reactants>>products)
# We leave the product side empty as the conditions encode the outcome
reaction_smiles = (
...
X_raw = np.concatenate([rxnfp_array, ligand_array, base_array, solvent_array], axis=1)
drop  = np.isnan(X_raw).any(0) | np.isinf(X_raw).any(0) | (np.var(X_raw, 0) == 0)
X = X_raw[:, ~drop]
y = df["yield_uv"].values / 100

np.save("X_features_rxnfp.npy", X)
np.save("y_target.npy", y)

print(f"Done ✅ — X: {X.shape}, y: {y.shape}")
print(f"RXNFP component: {rxnfp_array.shape[1]} dims | "
      f"Condition descriptors: {X.shape[1] - rxnfp_array.shape[1]} dims (after cleaning)")
